...
Each `Map_images` row records which plants table it belongs to, so a document can hold a
map per plot. Add a widget for each plants table and each finds its own row.
